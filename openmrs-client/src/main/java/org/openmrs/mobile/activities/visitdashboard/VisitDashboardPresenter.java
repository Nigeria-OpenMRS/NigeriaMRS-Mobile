...
public class VisitDashboardPresenter extends BasePresenter implements VisitDashboardContract.Presenter {
    ...
    @Override
    public void startFormDisplayActivityWithEncounter(Encounter encounter) {
        addSubscription(visitDAO.getVisitByID(visitId)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(visit -> {
                    Patient patient = visit.getPatient();
                    encounter.setPatient(patient);
                    encounter.setPatientUUID(patient.getUuid());
                    if(patient.getUuid() != null) {
                        mVisitDashboardView.startFormDisplayActivity(encounter);
                    }
                }));
