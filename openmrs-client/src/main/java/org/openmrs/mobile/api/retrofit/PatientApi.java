...
public class PatientApi extends RetrofitApi {
            ...
            dm.when(locationApi.getLocationUuid(), getIdGenPatientIdentifier(), getPatientIdentifierTypeUuid())
                    .done(new DoneCallback<MultipleResults>() {
                        ...
                        public void onDone(final MultipleResults results) {
                            ...
                            List<PatientIdentifier> identifiersPatients = new ArrayList<PatientIdentifier>();
                            identifiersPatients = patient.getIdentifiers();
                            for (PatientIdentifier p : identifiersPatients){
                                for (IdentifierType result : identifierTypesResults.getResults()){
                                    if (result.getDisplay().equals(p.getDisplay())) {
                                        ...
                                        identifier.setIdentifierType(result);
     ...
     * Update Patient
     */
    public void updatePatientIdentifier(final Patient patient,final PatientIdentifier identifier, @Nullable final DefaultResponseCallbackListener callbackListener) {
        if (NetworkUtils.isOnline()) {
            Call<PatientIdentifier> call = restApi.updateIdentifier(patient.getUuid(),identifier);
            call.enqueue(new Callback<PatientIdentifier>() {
                ...
                public void onResponse(Call<PatientIdentifier> call, Response<PatientIdentifier> response) {
                    if (response.isSuccessful()) {
                        PatientIdentifier updatedPatient = response.body();
